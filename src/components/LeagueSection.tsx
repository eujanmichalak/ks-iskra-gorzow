...
export const LeagueSection = () => {
    ...
    const fetchData = async () => {
        ...
        if (tableRes) {
            ...
            setLastUpdate(tableRes.last_update);
        }
        ...
        if (matchRes) {
            setLastMatches(matchRes.map(m => ({
                id: m.id, home: m.home, away: m.away,
                sH: m.score_home, sA: m.score_away, date: m.match_date
            })));
        }
        setLoading(false);
    };
    ...
    useEffect(() => {
        ...
        // REALTIME: Słuchamy zmian w obu tabelach
        const channel = supabase.channel('league-sync')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'league_table' }, () => fetchData())
            .on('postgres_changes', { event: '*', schema: 'public', table: 'match_history' }, () => fetchData())
            .subscribe();
    ...
    return (
        <section id="table" className="py-12 md:py-24 bg-white px-2 sm:px-4 md:px-6 text-black">
            <div className="max-w-7xl mx-auto grid lg:grid-cols-12 gap-8 md:gap-12 items-start">
                ...
                <div className="lg:col-span-8 flex flex-col w-full">
                    ...
                    <div className="border-[3px] md:border-4 border-black rounded-[24px] md:rounded-[40px] bg-white shadow-[10px_10px_0px_0px_rgba(0,0,0,0.05)] overflow-hidden">
                            ...
                            <tbody className="divide-y-2 divide-slate-100">
                                {tableData.map((row, idx) => (
                                    <tr key={idx} className={`${row.isUs ? 'bg-slate-50' : 'bg-white hover:bg-slate-50/50 transition-colors'}`}>
                                        ...
                                        <td className="p-3 md:p-7 font-black uppercase italic text-black text-xs md:text-lg leading-tight truncate">
                                            {row.team}
                                        </td>
                    ...
                    {lastUpdate && (
                        <div className="mt-4 md:mt-8 flex items-center justify-center md:justify-end gap-2 text-black font-black uppercase text-[9px] md:text-[10px] bg-slate-100 w-fit md:ml-auto mx-auto px-4 py-2 rounded-full border border-black/10">
                            <Clock size={12} /> Ostatnia aktualizacja: {lastUpdate}
                        </div>
                    )}
                </div>

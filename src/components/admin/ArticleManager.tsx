...
export const ArticleManager = () => {
    ...
    const [newArticle, setNewArticle] = useState({
        title: '', author: '', category: 'inne', description: '', image: ''
    });
    ...
    const saveArticles = (updated: Article[]) => {
        setArticles(updated);
        localStorage.setItem('iskra_articles', JSON.stringify(updated));
    };
    ...
    const addArticle = () => {
        ...
        setNewArticle({ title: '', author: '', category: 'inne', description: '', image: '' });
    };
            ...
            <div className="bg-white rounded-[32px] md:rounded-[40px] p-5 md:p-8 shadow-2xl border border-slate-100 text-slate-900">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                    ...
                    <div className="space-y-4 flex flex-col">
                        ...
                        <button onClick={addArticle} className="w-full mt-4 bg-slate-950 text-white py-4 md:py-5 rounded-xl md:rounded-[24px] font-[1000] uppercase italic tracking-widest hover:bg-slate-900 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3">
                            <Plus size={20} strokeWidth={3}/> <span className="text-sm md:text-base">Opublikuj artykuł</span>
                        </button>
                    </div>
                </div>
            </div>

            {/* SEKCJA 2: LISTA ARTYKUŁÓW */}
            <div className="bg-white rounded-[32px] md:rounded-[40px] p-5 md:p-8 shadow-2xl border border-slate-100 text-slate-900">
                        ...
                        articles.map(art => (
                            <div key={art.id} className="flex flex-col sm:flex-row sm:items-center gap-4 p-3 md:p-4 bg-white rounded-2xl md:rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                                ...
                                <div className="flex sm:block justify-end pt-2 sm:pt-0 border-t sm:border-none border-slate-50">
                                    <button onClick={() => saveArticles(articles.filter(a => a.id !== art.id))} className="p-2.5 text-slate-300 hover:text-red-500 transition-all hover:bg-red-50 rounded-xl">
                                        <Trash2 size={20} />
                                    </button>
                                </div>
                            </div>

...
export const Navbar = () => {
    ...
    // Ścieżka do Twojego lokalnego pliku w public/photos/
    const logoPath = "/photos/iskra.png";
        ...
        <nav className={`fixed top-0 inset-x-0 z-[100] transition-all duration-500 ${scrolled || mobileMenu ? 'bg-white/95 backdrop-blur-md py-3 shadow-2xl' : 'bg-transparent py-6'}`}>
            <div className="max-w-7xl mx-auto px-4 md:px-6 flex items-center justify-between">
                ...
                <Link href="/" className="flex items-center gap-2 md:gap-3 group shrink-0">
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border-2 border-iskra-red transition-transform group-hover:rotate-[10deg] group-hover:scale-110 shadow-lg bg-white flex items-center justify-center p-1">
                        <img
                            src={logoPath}
                            alt="Logo Iskra Gorzów"
                ...
                <div className="flex items-center gap-2">
                    <Link
                        href="/admin"
                        className={`flex items-center gap-2 px-3 md:px-5 py-2 md:py-2.5 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-widest transition-all shadow-lg active:scale-95 border-2 ${scrolled || mobileMenu

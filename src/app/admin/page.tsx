...
// Importujemy klienta supabase z Twojego pliku lib
import { supabase } from '@/lib/supabase';

export default function LoginPage() {
    const [email, setEmail] = useState(''); // Zmieniono z login na email
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (authError) {
            ...
            // Sukces - przekierowanie do pulpitu
            ...
            <div className="max-w-md w-full relative z-10">
                ...
                <div className={`bg-white/5 backdrop-blur-2xl border ${error ? 'border-iskra-red' : 'border-white/10'} p-10 rounded-[48px] shadow-2xl transition-all duration-300`}>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4 mb-2 block italic">E-mail Administratora</label>
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-iskra-red hover:bg-red-700 disabled:opacity-50 text-white font-[1000] uppercase italic tracking-widest py-5 rounded-2xl transition-all shadow-lg shadow-red-900/20 flex items-center justify-center gap-3 group active:scale-95"
                        >
                            {loading ? (

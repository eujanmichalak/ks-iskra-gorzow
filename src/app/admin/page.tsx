...
import { supabase } from '@/lib/supabase'; // Import Twojego klienta

export default function LoginPage() {
    const [email, setEmail] = useState(''); // Zmieniamy login na email (wymóg Supabase)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (authError) {
            ...
            router.refresh(); // Odśwież router, żeby złapał sesję
        }
    };
                ...
                <div className={`bg-white/5 backdrop-blur-2xl border ${error ? 'border-iskra-red' : 'border-white/10'} p-10 rounded-[48px] shadow-2xl transition-all duration-300`}>
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-iskra-red hover:bg-red-700 text-white font-[1000] uppercase italic tracking-widest py-5 rounded-2xl transition-all shadow-lg shadow-red-900/20 flex items-center justify-center gap-3 group active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {loading ? (
